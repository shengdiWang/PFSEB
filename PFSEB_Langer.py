...
def psi_H(zeta1, zeta2):
    ...
    if zeta1 < 0:
        ...
        res = 0.5 * zeta1 + 3.9 * zeta1 ** 2 - (0.5 * zeta2 + 3.9 * zeta2 ** 2)
...
df = test.SEB_RTS0()
df.to_csv(path.join(dir_mod, 'SEB_CH04_psy_Langer_CG_ori.csv'), index = False)
...
# mask = df_quad['date'] >= pd.to_datetime('20001001')
# mask *= (df_quad['date'] <= pd.to_datetime('20011001'))
# df_quad = df_quad[mask]


# df_ori = pd.read_csv(path.join(dir_mod, 'SEB_CH04_psy_Langer_CG_ori.csv'))
# df_ori['date'] = pd.to_datetime(df_ori['date'])
# df_ori = df_ori[mask]

# df_cg = pd.read_csv(path.join(dir_mod, 'SEB_CryoGrid.csv'))
