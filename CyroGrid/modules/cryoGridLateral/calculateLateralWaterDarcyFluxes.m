...
if PARA.ensemble.adjacency_water(index,j)>0
    ...
    if infiltration_condition_index && infiltration_condition_j
        ...
        [waterpotWindex, hasWater_index] = max([wt_index, inf_index ] );
        [waterpotWj, hasWater_j]         = max([wt_j,     inf_j     ] );
        
        % determine lateral hydraulic conductivity (surface/subsurface)
        if  ( hasWater_index==1 || hasWater_j==1 ) && ( max( [waterpotWindex, waterpotWj] ) > max( [ soil_index, soil_j ] ) )
        ...
        if (waterpotWj > waterpotWindex && hasWater_j==1)% Current worker is gaining water
            ...
            %contact_height = waterpotWj - max( [ waterpotWindex, inf_j ]);
            contact_height = waterpotWj - max( [ inf_index, inf_j ]);
        ...
        elseif (waterpotWindex > waterpotWj && hasWater_index==1) % Current worker is loosing water
            ...
            %contact_height =waterpotWindex - max( [ waterpotWj, inf_index ] );
            contact_height =waterpotWindex - max( [ inf_index, inf_j ] );
            %Distance=sqrt(DeltaH^2 + PARA.ensemble.hydraulicDistance(j,index)^2);
            section=contact_height .* PARA.ensemble.hydraulic_contact_length(index,j);      % important to have (index,j) in case of asymmetric hydraulic contact length!!!
