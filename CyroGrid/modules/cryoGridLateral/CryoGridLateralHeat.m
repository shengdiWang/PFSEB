    ...
    if precondition_heatExchange
        ...
        for j=1:numlabs
            if PARA.ensemble.adjacency_heat(labindex,j)
                ...
                TEMPORARY.dE_tot_lateral(j) = TEMPORARY.dE_tot_lateral(j) + sum(  F_lateral_j ./ PARA.location.area .* PARA.technical.syncTimeStep.*24.*3600 ); % depth intergrated in [ J/m^2 ]
        ...
        BALANCE.energy.Q_lateral = BALANCE.energy.Q_lateral + sum(  dE_dt_lateral ) .* PARA.technical.syncTimeStep.*24.*3600 ; % in [ J / m^2 ]
