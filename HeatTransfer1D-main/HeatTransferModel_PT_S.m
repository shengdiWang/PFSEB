...
%% ===== SETTING-UP =======================================================
% ---- directory ----
dir_mod = '/Users/shengdiwang/Library/CloudStorage/OneDrive-个人/桌面/CMA/SIMnew';
...
for si = 1:length(scSites)
    ...
    AIRT   = foc.sat; % air temperature
    SND    = foc.HS;    % snow depth
            ...
            for yr = 1:spinTime
                ...
                for NDAYI = 1:length(dateRange1)
                    ...
                    if (SNOWH > 0)
                        ...
                        albei = snowAlbedo_cryogrid(SNOWH_yday, SNOWH, AIRT(dayi), albei);
                        melt_d = melt_d + 1;
                    else
                        albei = albedoG;
                    end
...
%                         end
                        ...
                        if (SNOWH < 0.003)
                            ...
                            QE    = - (alpha * delta * (AQSI + QLI + Qle + QC) / (delta + gama));
                            ra    = log(wsHeight / rz(dayi)) * log(wsHeight / rz(dayi)) / (VONK^2 * wsi);
                        else
                            delta = 4098 * atmosphericVaporPressure(TA) / 1E3 / ((TA - 273.15 + 237.3) ^ 2);
                            gama  = 0.00163 * PA / 1E3 / (1E3 * (2500.8 - 2.36 * (TA - 273.15)) / 1E6);
                            QE    = - (0.95 * delta * (AQSI + QLI + Qle + QC) / (delta + gama));
                        end

                        % CALL SUBDXQE
                        if (SNOWH < 0.003)
                            DXQE  = - alpha * delta * (DXQle + DXQC) / (delta + gama);
                        else
                            DXQE  = - 0.95 * delta * (DXQle + DXQC) / (delta + gama);
                        end
                        %%
