...
dir_foc = '/Users/shengdiwang/Desktop/CMA/FOC';
...
latSub   = 2.838E6; % latent heat of ice sublimation [J kg-1]
        ...
        for thetaU1 = thetaU1
            ...
            for yr = 1:spinTime
                ...
                for NDAYI = 1:length(dateRange1)
                    ...
                    wsi    = WS(dayi);
                    PA     = PRE(dayi);
                    EA     = RH(dayi) * 0.01 * atmosphericVaporPressure(TA);
            ...
            for NDAYI = 1:length(dateRange2)
                ...
                while EES < REEE
                    ...
                    % CALL SUBQE [Eq.6]
                    QE = roAir * latSub * DE * XI * (0.622 * (EA - ES0) / PA);
                    q  = (0.622 * EA / PA) / (0.622 * EA / PA + 1);
                    qS0  = (0.622 * ES0 / PA) / (0.622 * ES0 / PA + 1);
                    QE = roAir * latSub * DE * XI * (q - qS0);
...
%             hold off
            ...
            fprintf(fileID, fmt, 'date', 'gst', 'gst2', 'wu', 'QE', 'QH', 'XI', 'RI', 'EA');
            fmt = '%s %6.2f %6.2f %6.2f %6.2f %6.2f %6.2f %6.2f %6.2f\n';
            ...
            for k = 1:length(dateRange3)
                ...
                fprintf(fileID, fmt, char(dateRange3(k)), ...
                    resGST(7, k), resGST(15, k), resthetaU(7, k), resQE(k), resQH(k), resXI(k), resRI(k), resEA(k));
