...
class SEB(object):
    ...
    def SEB_RTS0(self):
         ...
         for NDAYI in range(0, len(self.dateRange2)):
            ...
            # DH & DE
            DH    = VONK**2 * wsi / (math.log(wsHeight / RZ))**2
            DE    = DH
            
            
            # Ri
            RI    = GRAVIT * wsHeight * (TA - TS) / (TA * wsi ** 2)
...
df = test.SEB_RTS0()
df.to_csv(path.join(dir_mod, 'SEB_CHO4_psy_CG_LF.csv'), index = False)
...
df_LF = pd.read_csv(path.join(dir_mod, 'SEB_CHO4_psy_CG_LF.csv'))
...
plt.plot(df_quad.date, df_quad.QE, 'y', lw = 0.5)
plt.plot(df_quad.date, df_ori.QE, 'g', lw = 0.5)
plt.plot(df_LF.date, - df_LF.QE, 'r', lw = 0.5)


plt.plot(test.foc['date'], test.foc['satFinal'] * 3, 'k', lw = 0.5)

plt.xlim(pd.to_datetime('20000101'), pd.to_datetime('20001231'))
plt.ylim(-50, 180)
...
# plt.plot(df_quad.date, df_ori.QH, 'g', lw = 0.5)


# # plt.plot(df_cg.index, df_cg.RTS0, 'r', lw = 0.5)
# plt.plot(df_quad.date, df_quad.RTS0, lw = 0.5)
# plt.plot(df_quad.date, df_ori.RTS0, 'g', lw = 0.5)
